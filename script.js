...
fetch(`${url}?skip=${(currentPage - 1) * itemsPerPage}&limit=${itemsPerPage}`)
    ...
    .catch((error) => {
        ...
        headline.textContent = "Oops... Something's wrong :(";
        console.log(error);
    })
    .finally(() => {
        closeLoader();
...
function renderCard(user) {
    ...
    // like button 
    let localStorageCard = "likedCount" + user.id;
    let likeCount = localStorage.getItem(localStorageCard) || 0;
    likeButton.innerHTML = "Liked - " + likeCount;

    likeButton.addEventListener("click", function () {
        likeCount++;
        likeButton.innerHTML = "Liked - " + likeCount;
        localStorage.setItem(localStorageCard, likeCount);
    });


    // after "click image" -- open new details 
